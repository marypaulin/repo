...
def bbound(x, y, lamb, prior_metric = None, MAXDEPTH = 4, niter=float('Inf')):#
        ...
        for i in range(len(d)):
            ...
            for j in range(1, nrule+1):
                if (j not in d0)and(-j not in d0):
                    ...
                    cap_l[0], incorr_l[0] = Cache_l1.num_captured, Cache_l1.num_captured_incorrect, 
                    
                    if l2_sorted not in leaves:
                        leaves[l2_sorted] = CacheLeaf((l2,),x,y,i_points)
                    
                    Cache_l2 = leaves[l2_sorted]
                    cap_l[1], incorr_l[1] = Cache_l2.num_captured, Cache_l2.num_captured_incorrect
                    ...
                    tree_new = CacheTree(x = x, y = y, prefix = t,
                                         ...
                                         splitleaf = sl,
                                         lbound = new_lbound,
                                         p = tree.p[:i]+tree.p[i+1:]+[Cache_l1.p,Cache_l2.p],
                                         B0 = tree.B0[:i]+tree.B0[i+1:]+[Cache_l1.B0,Cache_l2.B0],
                                         lamb = lamb,
                                         prior_metric=prior_metric,
                                         points_cap = tree.points_cap[:i]+tree.points_cap[i+1:]+[Cache_l1.points_cap,Cache_l2.points_cap],
                                         prediction = tree.prediction[:i]+tree.prediction[i+1:]+[Cache_l1.prediction,Cache_l2.prediction]
                                        )
    ...
    fname = "_".join([str(nrule),str(ndata),prior_metric,str(lamb),str(MAXDEPTH),".txt"])
...
#all data, 5 features
bbound(x, y, lamb=0.001, prior_metric="gini", MAXDEPTH = 4)
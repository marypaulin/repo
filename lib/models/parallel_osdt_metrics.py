...
class ParallelOSDT:
    ...
    def __init__(self,
        ...
        self.metric_dex = 0
        self.max_switch = self.configuration['switch_time']
    ...
    def snapshot(self):
        ...
        if self.convergence_logger == None:
            self.convergence_logger = Logger(path='data/convergence/worker_{}.csv'.format(self.worker_id),
                                             header=['time', 'lowerbound', 'upperbound'])
        ...
        if self.distribution_logger == None:
            self.distribution_logger = Logger(path='data/distribution/worker_{}.csv'.format(self.worker_id),
                                              header=['time', 'local_queue_length', 'global_queue_length'])
        self.distribution_logger.log([self.elapsed_time(), self.tasks.length(), self.tasks.length(local=False)])

        if self.memory_logger == None:
            self.memory_logger = Logger(path='data/memory/worker_{}.csv'.format(self.worker_id),
                                        header=['time', 'local_table_size'])
    ...
    def task(self, worker_id, services):
        ...
        while not self.complete() and not self.timeout():  # and peers.value == self.workers:
            ...
            print('Worker {} has seen {} problems and {} trees'.format(self.worker_id, len(self.results),
                                                                       self.results[self.root].count))
        ...
        self.print(
            'Worker {} Finishing (Complete: {}, Timeout: {})'.format(self.worker_id, self.complete(), self.timeout()))

        return self.output() if self.complete() else None

    def prioritize(self, capture, path):
        priority_metric = self.configuration['priority_metric'][self.metric_dex]
        ...
        if not key in self.results:
            ...
            if (self.configuration['accuracy_lowerbound'] and bounding_interval.lowerbound > support):
                ...
                result = Result(optimizer=optimizer, optimum=optimum)
                  ...
                  (self.configuration[
                       'support_lowerbound'] and 0.5 * support <= self.lamb) or  # Insufficient support for splitting
                  (self.configuration[
                       'incremental_accuracy_lowerbound'] and bounding_interval.uncertainty <= self.lamb)):  # Insufficient potential accuracy gain for splitting
    ...
    def dependencies(self, task, current_result):
        ...
        minimum_bounding_interval, minimizing_split, relevant_splits, irrelevant_splits = self.minimize_choice_of_split(
            capture, path)
        ...
        count = sum((1 if not d in self.results else self.results[d].count) for d in set(sum(
            [self.dataset.split(j, capture=task.capture) for j in (tuple(relevant_splits) + tuple(irrelevant_splits))],
            ())))
        ...
        if (self.configuration['accuracy_lowerbound'] and minimum_bounding_interval.lowerbound > support):
            ...
            if self.configuration['independence'] < 1 and (
                    self.configuration['independence'] == 0 or random() >= self.configuration[
                'independence']) and current_result.running:
                dependencies = tuple()
            else:
    ...
    def minimize_choice_of_split(self, capture, path):
        ...
        if self.configuration['look_ahead']:
            ...
            relevant_splits = (j for j in self.dataset.gini_index if bounding_intervals[
                j].lowerbound <= minimum_bounding_interval.upperbound and not j in path)
            irrelevant_splits = (j for j in self.dataset.gini_index if bounding_intervals[
                j].lowerbound > minimum_bounding_interval.upperbound and not j in path)  # SLOW
        else:
    ...
    def switch_metric_timeout(self):
        if self.elapsed_time() - self.switch_time > self.max_switch:
            self.switch_time = time()
            self.metric_dex = (self.metric_dex + 1) % len(self.configuration['priority_metric'])
            return True
        return False
    ...
    def output(self):
        return Tree(self.root, self.results, self.dataset,
                    capture_equivalence=self.configuration['capture_equivalence'])
    ...
    def solve(self, workers=1, visualize_model=False, visualize_training=False):
        ...
        if self.configuration['similarity_threshold'] > 0 and self.configuration['similarity_threshold'] != float(
                'Inf'):
        ...
        if model != None:
            ...
            if self.verbose or self.log:
                self.print(
                    "ParallelOSDTError: Early Termination after {} seconds".format(round(self.elapsed_time(), 3)))
            raise Exception(
                "ParallelOSDTError: Early Termination after {} seconds".format(round(self.elapsed_time(), 3)))

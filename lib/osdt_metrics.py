...
class Tree:
    ...
    def set_metric(self, prior_metric, ndata):
        ...
        if leaves[0].num_captured == ndata:
            ...
            if num_cap_rm < ndata:
                self.metric = sum(self.entropy[i] for i in range(l) if self.splitleaf[i] == 0) / (
                        ndata - sum(leaf.num_captured for leaf in removed_leaves))
            else:
                self.metric = sum(self.entropy[i] for i in range(l) if self.splitleaf[i] == 0) / 0.01
            ...
            if num_cap_rm < ndata:
                self.metric = sum(self.giniindex[i] for i in range(l) if self.splitleaf[i] == 0) / (
                        ndata - sum(leaf.num_captured for leaf in removed_leaves))
            else:
                self.metric = sum(self.giniindex[i] for i in range(l) if self.splitleaf[i] == 0) / 0.01
...
def switch_metrics(new_metric, queue):
    new_queue = []
    for met, tree in queue:
        tree.set_metric(new_metric)
        heapq.heappush(new_queue, (tree.metric, tree))
    return new_queue
...
def bbound(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
           ...
           saveTree = False, readTree = False, switch_time=float('Inf')):
    """
    ...
    tree0 = Tree(cache_tree=d_c, lamb=lamb,
                 ndata=ndata, splitleaf=[1], prior_metric=prior_metric[metric_dex])
    ...
    if readTree:
        ...
        tree_p = Tree(cache_tree=d_c, lamb=lamb,
                     ndata=ndata, splitleaf=[1]*len(d_c.leaves), prior_metric=prior_metric[metric_dex])
    ...
    if logon:
        ...
        fname = "_".join([str(nrule), str(ndata), prior_metric[metric_dex],
        ...
        for leaf_rules in product(*rules_for_leaf):
            ...
            for new_leaf_split in new_leaf_splits:
                ...
                tree_new = Tree(cache_tree=child, ndata=ndata, lamb=lamb,
                                splitleaf=new_leaf_split, prior_metric=prior_metric[metric_dex])
    ...
    print("prior_metric=", prior_metric[metric_dex])

...
class ParallelOSDT:
    ...
    def task(self, worker_id, services):
        ...
        while not self.terminate() and self.elapsed_time() <= self.max_time and not ParallelOSDT.interrupt:
            ...
            if task == None:
                if self.verbose or self.log:
                    self.print("Worker {} Idle {}".format(self.worker_id, self.terminate()))
                # results.__refresh__()
                continue
    ...
    def minimize_choice_of_split(self, capture, path):
        ...
        if self.configuration['look_ahead']:
            ...
            irrelevant_splits = (j for j in self.dataset.gini_index if bounding_intervals[j].lowerbound > minimum_bounding_interval.upperbound and not j in path)
        else:

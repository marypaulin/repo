...
class CacheTree:
    def __init__(self, name, P, N, lamb, leaves, w=None, theta=None):
        ...
        if name != 'auc_convex' and name != 'partial_auc':
            ...
            _, _, _, loss = partial_auc(self.leaves, self.P, self.N, self.theta)
            self.risk = loss+lamb*self.H
...
class Tree:
    def __init__(self, cache_tree, n, lamb, splitleaf=None, prior_metric=None):
        ...
        if cache_tree.name != 'auc_convex' and cache_tree.name != 'partial_auc':
            ...
            fp = np.array([0, 0])
            ordered_fixed = [l for l in ordered_leaves if l not in split_set[0]]
        ...
        elif cache_tree.name == 'partial_auc':
            split_set = []
            split_set.append([leaves[i] for i in range(self.H) if splitleaf[i]==1])
            split_p = sum([leaves[i].p for i in range(self.H) if splitleaf[i]==1])
            split_n = sum([leaves[i].n for i in range(self.H) if splitleaf[i]==1])
            ordered_leaves = order_leaves(leaves)
            tp = np.array([0, split_p], dtype=float)
            fp = np.array([0, 0], dtype=float)
            ordered_fixed = [l for l in ordered_leaves if l not in split_set[0]]
            if len(ordered_fixed) > 0:
                i = 0
                '''
                while fp[i+1] < cache_tree.N * cache_tree.theta and i < len(ordered_fixed):
                    tp = np.append(tp, tp[i+1]+ordered_fixed[i].p)
                    fp = np.append(fp, fp[i+1]+ordered_fixed[i].n)
                    i += 1
                    tp[i+1] = ((tp[i+1]-tp[i])/(fp[i+1]-fp[i]))*(cache_tree.N*cache_tree.theta-fp[i+1])+tp[i+1]
                    fp[i+1] = cache_tree.N*cache_tree.theta
                    self.lb = 1- 0.5*sum([(tp[i]+tp[i-1])*(fp[i]-fp[i-1])/(cache_tree.P*cache_tree.N) for i in range(1,len(tp))]) + lamb*self.H
                '''
                
                while fp[i+1] < cache_tree.N*cache_tree.theta:
                     if i < len(ordered_fixed):
                         tp = np.append(tp, tp[i+1]+ordered_fixed[i].p)
                         fp = np.append(fp, fp[i+1]+ordered_fixed[i].n)
                         i += 1
                     else:
                         tp = np.append(tp, tp[i+1]+0)
                         fp = np.append(fp, fp[i+1]+split_n)
                         break                         
                tp[len(tp)-1] = ((tp[len(tp)-1]-tp[len(tp)-2])/(fp[len(fp)-1]-fp[len(fp)-2]))*\
                                 (cache_tree.N*cache_tree.theta-fp[len(fp)-1])+tp[len(tp)-1]
                fp[len(fp)-1] = cache_tree.N*cache_tree.theta
                self.lb = 1- 0.5*sum([(tp[i]+tp[i-1])*(fp[i]-fp[i-1])/\
                                       (cache_tree.P*cache_tree.N) \
                                       for i in range(1,len(tp))]) + lamb*self.H
            else:
                #self.lb = 1-split_p*cache_tree.theta/cache_tree.P + lamb*self.H
                self.lb = 1-cache_tree.theta + lamb*self.H
...
def partial_auc(leaves, P, N, theta):
    ordered_leaves = order_leaves(leaves)
    tp = fp = np.array([0], dtype=float)
    if len(leaves) > 1:
        i = 0
        while fp[i] < N*theta and i < len(leaves):
            tp = np.append(tp, tp[i]+ordered_leaves[i].p)
            fp = np.append(fp, fp[i]+ordered_leaves[i].n)
            i += 1
        tp[i] = ((tp[i]-tp[i-1])/(fp[i]-fp[i-1]))*(N*theta-fp[i])+tp[i]
        fp[i] = N*theta
    else:
        tp = np.append(tp, P*theta)
        fp = np.append(fp, N*theta)
    loss = 1-0.5*sum([(tp[i]/P+tp[i-1]/P)*(fp[i]/N-fp[i-1]/N) for i in range(1,len(tp))])
    return ordered_leaves, tp, fp, loss
...
def generate_new_splitleaf(name, P, N, unchanged_leaves, removed_leaves, new_leaves, lamb,
    ...
    if name == 'partial_auc':
        ordered_leaves, tp, fp, ld = partial_auc(unchanged_leaves+removed_leaves, P, N, theta)
    ...
    for i in range(n_removed_leaves):
        ...
        bound = Objective(name)
        if (name != 'f1') & (name != 'auc_convex') & (name != 'partial_auc'):
            ...
            removed = removed_leaves.copy()
            removed.remove(removed_leaves[i])
            ordered_leaves, tp, fp, ld_new = partial_auc(unchanged_leaves+removed+
                                         [new_leaves[idx1]]+[new_leaves[idx2]], P, N, theta)
            a = ld-ld_new
...
def bbound(x, y, name, lamb, prior_metric=None, w=None, theta=None, MAXDEPTH=float('Inf'), 
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        if (name != 'auc_convex') and (name != 'partial_auc') and (bound.loss(P, N, tree.FPu+delta_fp, tree.FNu+delta_fn, w) + n_removed_leaves * lambbb >= R_c):
            continue
        
        if (name == 'auc_convex' or name == 'partial_auc') and (tree.lb >= R_c):
            continue
        ...
        for leaf_rules in product(*rules_for_leaf):
            ...
            sl = generate_new_splitleaf(name, P, N, unchanged_leaves, removed_leaves, new_leaves,
                                        lamb, R_c, incre_support, w, theta) # incre_support is a_j
            # print("sl:", sl)
            ''' 
            ...
            if name == 'acc':
                ...
                ordered_leaves, tp, fp, loss = partial_auc(new_tree_leaves, P, N, theta)
                cannot_split = []
                for l in new_tree_leaves:
                    if l.p==0 or l.n==0:
                        cannot_split.append(True)
                        ...
                        while fp[i+1] < N*theta:
                            if i < len(leaves):
                                ...
                                tp = np.append(tp, tp[i+1]+0)
                                fp = np.append(fp, fp[i+1]+l.n)
                                break
                                
                        tp[len(tp)-1] = ((tp[len(tp)-1]-tp[len(tp)-2])/(fp[len(fp)-1]-fp[len(fp)-2]))*(N*theta-fp[len(fp)-1])+tp[len(tp)-1]
                        fp[len(fp)-1] = N*theta
                        
                        loss_new = 1- 0.5*sum([(tp[i]+tp[i-1])*(fp[i]-fp[i-1])/(P*N) for i in range(1,len(tp))])
                        delta_loss = loss - loss_new
                        if len(l.rules)>=MAXDEPTH or delta_loss<lamb or all([l.is_feature_dead[r - 1] for r in range(1, m + 1) if r not in map(abs, l.rules)]):
                            cannot_split.append(True)
                        else:
            ...
            for new_leaf_split in new_leaf_splits:
                ...
                '''
                tree_new = CacheTree(name, n, P, N, lamb, new_tree_leaves, new_leaf_split, prior_metric, w, theta)
                '''
...
def predict(name, leaves_c, prediction_c, dic, x, y, best_is_cart, clf, w=None, theta=None):
    """
    :param leaves_c:
    :param dic:
    :return:
    ...
    if best_is_cart:
        ...
        if name == 'acc':
            ...
            nodes = clf.apply(x)
            metric = np.empty([3, len(leaves_c)], dtype=float)
            ...
            fp = [0]
            i = 1
            while fp[i-1] < N*theta and i <= len(leaves_c)+1:

...
def cart(x, y, name, n, P, N, lamb, w, theta, MAXDEPTH):
    ...
    if name == 'acc':
        ...
        trainout_CART = (n_fp + w*n_fn)/(n)
    elif (name == 'auc') or (name == 'auc_convex'):
        trainout_CART = sklearn.metrics.roc_auc_score(y, pred_CART)
    elif name == 'f1':
        ...
        trainout_CART = 0.5*sum([(metric[0,i]/P+metric[0,i-1]/P)*(metric[1,i]/N-metric[1,i-1]/N) for i in range(1,nleaves_CART+1)])
        ...
        fp = [0]
        i = 1
        while fp[i-1] < N*theta and i <= nleaves_CART+1:

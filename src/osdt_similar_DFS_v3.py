...
class Tree:
    ...
    def __init__(self, cache_tree, ndata, lamb, stackposition, splitleaf=None, prior_metric=None):
        ...
        self.minobj = cache_tree.risk  # the minimum objective of all its child trees
...
def bbound_DFS_v3(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
           ...
           saveTree = False, readTree = False, distance=10):
    """
    ...
    tree0 = Tree(cache_tree=d_c, lamb=lamb,
                 ndata=ndata, splitleaf=[1], prior_metric=prior_metric, stackposition=0)
    ...
    if readTree:
        ...
        tree_p = Tree(cache_tree=d_c, lamb=lamb,
                     ndata=ndata, splitleaf=[1]*len(d_c.leaves), prior_metric=prior_metric, stackposition=len(queue))
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        for key, tree_list in similar_table.items():
            key_tree = tree_list[0]
            if key_tree.flag == 0 and len(queue) <= key_tree.stackposition:
                key_tree.flag = 1
                break
        ...
        if simil_support:
            flag_simil = 0
            for i in range(len(tree.similarto)):
                t_simil = tree.similarto[i]
                if t_simil.minobj - tree.omega[i] + tree.lb - t_simil.lb >= R_c:
                    flag_simil = 1
                    break
            if flag_simil:
                continue
        ...
        for leaf_rules in product(*rules_for_leaf):
            ...
            for key, tree_list in similar_table.items():
                key_tree = tree_list[0]
                if key_tree.flag == 0 and R < key_tree.minobj:
                    key_tree.minobj = R
            ...
            for new_leaf_split in new_leaf_splits:
                ...
                tree_new = Tree(cache_tree=child, ndata=ndata, lamb=lamb,
                                splitleaf=new_leaf_split, prior_metric=prior_metric, stackposition=len(queue))
                ...
                new_n_removed_leaves = sum(new_leaf_split)
                if new_n_removed_leaves == 1:
                    s_leaf = list(compress(new_tree_leaves, new_leaf_split))[0]
                    new_key = s_leaf.points_cap

                    for key, tree_list in similar_table.items():
                        omega = rule_vxor(key, new_key)
                        key_tree = tree_list[0]
                        #print(">>>>>>>>>>>>>>>>>>>> omega:", omega)
                        if omega <= distance:
                            tree_new.similarto.append(key_tree)
                            tree_new.omega.append(omega)
                            tree_list.append(tree_new)

                        #print(">>>>>>>> key:", key)
                        #print(">>>>>>>> tree_list:", tree_list)

                    if not new_key in similar_table:
                        similar_table[new_key] = [tree_new]

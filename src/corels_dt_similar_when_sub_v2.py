...
def generate_new_splitleaf(tree_new_leaves, sorted_new_tree_rules, leaf_cache, splitleaf_list, ndata, nleaves, lamb,
...
def corr_dic(x, corr_threshold):
    ...
    for i in range(s1):
        for j in range(i + 1, s2):
            if corr_df[i][j] == True:
                related_features.append(i+1)
                related_features.append(j+1)

    return related_features

def bbound_similar_when_sub(x, y, lamb, prior_metric=None, MAXDEPTH=4, niter=float('Inf'), logon=False,
    ...
    related_features = corr_dic(x, 0.3)
        ...
        for i in range(len(leaves)):
            ...
            for j in range(1, nrule + 1):
                ...
                if j not in d0 and -j not in d0:
                    ...
                    l1_similar_leafdead = 0
                    l2_similar_leafdead = 0

                    ###### similar support bound ######
                    if j in related_features:
                        l1_dead = Cache_l1.is_dead
                        l2_dead = Cache_l2.is_dead


                        if l1_dead:
                            lb = new_lbound[-2]
                            pc = Cache_l1.points_cap
                            # cache the lower bound of the prefix, and the points not captured by the prefix
                            if (lb, pc) not in deadprefix_cache:
                                # deadprefix_cache.append((lb, pc))
                                deadprefix_cache = [(lb, pc)] + deadprefix_cache
                    ...
                    sl = generate_new_splitleaf(
                        tree_new_leaves, sorted_new_tree_rules, leaf_cache, splitleaf_list, ndata,
                        len(unchanged_leaves) + 2, lamb, min(R_c, new_lbound[-1] + loss_l2),
                    ...
                    tree_new = CacheTree(ndata=ndata, leaves=tree_new_leaves,
                                         ...
                                         splitleaf=sl,
                                         lbound=new_lbound,
                                         similar_leafdead=tree.similar_leafdead[:i] + tree.similar_leafdead[i + 1:] + [
                                             l1_similar_leafdead, l2_similar_leafdead]
                                         )

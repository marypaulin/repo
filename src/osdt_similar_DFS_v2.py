...
def bbound_DFS_v2(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
    ...
    version 2 -- within the DFS framework, we sort the siblings
    """
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        for leaf_rules in product(*rules_for_leaf):
            ...
            heapq.heappush(temp_children, (child.risk, child))
            ...
            heapq.heappush(temp_stackpositions, (child.risk, len(queue)))
        ...
        while temp_children_p:
            temp_children.append(heapq.heappop(temp_children_p))
        while temp_stackpositions_p:
            temp_stackpositions.append(heapq.heappop(temp_stackpositions_p))

        #print("temp_children",temp_children)
        ...
        if n_removed_leaves == 1 and len(rules_for_leaf[0]) > 1:
            ...
            for i in range(n_children):
                _, t1 = temp_children[i]

                # use stack size to identify we have explored all the child trees
                t_similarity.append(Similarity(t1.risk, temp_stackpositions[i][1]))

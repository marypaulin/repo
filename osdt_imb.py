...
def order_fixed_leaves(leaves, splitleaf, H):
    split_set = []
    split_set.append([leaves[i] for i in range(H) if splitleaf[i]==1])
    ordered_leaves = order_leaves(leaves)
...
def bbound(x, y, name, lamb, prior_metric=None, w=None, theta=None, MAXDEPTH=float('Inf'), 
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        # not implement equivalent bound yet 
        if (name == 'auc_convex' or name == 'partial_auc') and (tree.lb + n_removed_leaves * lambbb>= R_c):
            continue
    ...
    return leaves_c, pred_c, dic, nleaves, m, n, totaltime, time_c, COUNT, C_c, \
            accu, best_is_cart, clf


def predict(name, leaves_c, prediction_c, nleaves, dic, x, y, best_is_cart, clf, w=None, theta=None):
    """
    :param leaves_c:
    :param dic:
    :return:
